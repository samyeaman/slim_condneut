...
//NOTE: modified by Sam Yeaman at locations flagged by comments with "CHANGED SY". 
...
class mutation_type
{
  ...
  //              x: gamma distribution with GxE interacton (reverse selection coefficient in the opposite population) //CHANGED SY
  //              c: gamma distribution that is conditionally neutral in patch1  //CHANGED SY
  //              d: gamma distribution that is conditionally neutral in patch2  //CHANGED SY
  //
...
public:
  ...
  mutation_type(float H, char D, vector<double> P)
  {
    ...
    if(s.find(d)==string::npos)  { cerr << "ERROR (initialize): invalid mutation type parameters" << endl; exit(1); } //CHANGED SY
  ...
  float draw_s()
  {
    switch(d)
      ...
      case 'x': return gsl_ran_gamma(rng,p[1],p[0]/p[1]); //CHANGED SY 
      case 'c': return gsl_ran_gamma(rng,p[1],p[0]/p[1]); //CHANGED SY
      case 'd': return gsl_ran_gamma(rng,p[1],p[0]/p[1]); //CHANGED SY
...
public:
  ...
  void update_fitness(chromosome& chr, int pit) //CHANGED SY
  {
      
      
    // calculate fitnesses in parent population and create new lookup table
    gsl_ran_discrete_free(LT);
    double A[(int)(G_parent.size()/2)]; for(int i=0; i<(int)(G_parent.size()/2); i++) { A[i] = W(2*i,2*i+1,chr, pit); }
    LT = gsl_ran_discrete_preproc((int)(G_parent.size()/2),A);
  }


  double W(int i, int j, chromosome& chr, int pit)
  {
	...
	while(pi != pi_max && (pj == pj_max || (*pi).x < (*pj).x))
	  {
	    //if((*pi).s != 0) { w = w*(1.0+chr.mutation_types.find((*pi).t)->second.h*(*pi).s); } //CHANGED SY
                   
        if((*pi).s != 0){
            //these are used to switch among mutation types: //CHANGED SY
            if (chr.mutation_types.find((*pi).t)->second.d == 'x' && pit == 1){
                //the X mutations are opposite selected in population 1 and normally selected in population 2:
                w = w*(1.0-chr.mutation_types.find((*pi).t)->second.h*(*pi).s);
            } else if (chr.mutation_types.find((*pi).t)->second.d == 'c' && pit == 1){
                //the C mutations are selected normally in population 2 and have neutral effects in population 1
                w = w; 
            } else if (chr.mutation_types.find((*pi).t)->second.d == 'd' && pit == 2){
                //the D mutations are selected normally in population 1 and have neutral effects in population 2
                w = w; 
            } else {
                w = w*(1.0+chr.mutation_types.find((*pi).t)->second.h*(*pi).s); //CHANGED SY
            }
	...
	while(pj != pj_max && (pi == pi_max || (*pj).x < (*pi).x))
	  {
	    //if((*pj).s != 0) { w = w*(1.0+chr.mutation_types.find((*pj).t)->second.h*(*pj).s); } //CHANGED SY
         
          
          if((*pj).s != 0){
              //these are used to switch among mutation types: //CHANGED SY
              if (chr.mutation_types.find((*pj).t)->second.d == 'x' && pit == 1){
                  //the X mutations are opposite selected in population 1 and normally selected in population 2:
                  w = w*(1.0-chr.mutation_types.find((*pj).t)->second.h*(*pj).s);
              } else if (chr.mutation_types.find((*pj).t)->second.d == 'c' && pit == 1){
                  //the C mutations are selected normally in population 2 and have neutral effects in population 1
                  w = w;
              } else if (chr.mutation_types.find((*pj).t)->second.d == 'd' && pit == 2){
                  //the D mutations are selected normally in population 1 and have neutral effects in population 1
                  w = w;
              } else {
                  w = w*(1.0+chr.mutation_types.find((*pj).t)->second.h*(*pj).s); //CHANGED SY
              }
			...
			if((*pi).t == (*temp_j).t && (*pi).s == (*temp_j).s) 
			  {
                ...
                if (chr.mutation_types.find((*pi).t)->second.d == 'x' && pit == 1){
                    ...
                    w = w*(1.0+(*pi).s); //CHANGED SY
                }
                homo = 1;
                
                
                  
			    //w = w*(1.0+(*pi).s); homo = 1; //CHANGED SY
			  }
			temp_j++;
		      }
...
public: 
  ...
  void swap_generations(int g, chromosome& chr)
  {
    ...
    for(it = begin(); it != end(); it++) 
      { 
	it->second.swap();
	it->second.update_fitness(chr, it->first); //CHANGED SY
      ...
      if(line.find('#') != string::npos) 
	{
	  if(line.find("MUTATION RATE") != string::npos) { get_line(infile,line);
	      ...
	      if(line.length()>0)
		{
		  ...
		  if(iss.eof()) { good = 0; } iss >> sub; 
		  if(sub.find_first_not_of("fgexcd") != string::npos )          { good = 0; } // DFE-type
...
void initialize_from_file(population& P, const char* file, chromosome& chr)
{
  ...
  for(P.it=P.begin(); P.it!=P.end(); P.it++) { P.it->second.update_fitness(chr,P.it->first); } //CHANGED SY
};
